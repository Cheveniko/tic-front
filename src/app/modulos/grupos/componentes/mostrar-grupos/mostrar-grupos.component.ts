...
export class MostrarGruposComponent implements OnInit, AfterViewInit {
  ...
  cargarGrupos(idNivel: string) {
    Swal.showLoading();
    this.gruposService.obtenerGruposPorIdNivel(idNivel).subscribe({
      next: (data) => {
  ...
  cargarNiveles(idCarrera: string) {
    Swal.showLoading();
    this.nivelesService.obtenerNivelesPorCarrera(idCarrera).subscribe({
      next: (data) => {
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        ...
        try {
          for (
            let index = 1;
            index <= this.carreraSeleccionada?.duracion!!;
            index++
          ) {
            Swal.showLoading();
            console.log({
            ...
            await lastValueFrom(
              this.nivelesService.crearNivel({
                idCarrera: this.carreraSeleccionada?.id!!,
                nombre: `Nivel ${index} - ${this.carreraSeleccionada?.codigo!!}`,
              }),
            );
