...
export class RestriccionLugarTiempoComponent implements OnInit {
  ...
  espacioFisicoSeleccionado?: ObtenerEspacioFisico = {
    id: "",
    nombre: "",
    aforo: 0,
  };
  ...
  cargarFormulario() {
    const espacioFisicoInicial = this.restriccionesPorActividad.length
      ? this.restriccionesPorActividad[0].espacioFisico
      : "";

    // Busco el objeto
    const espacioFisicoInicialObject = this.espaciosFisicosDisponibles.filter(
      (e) => e.nombre === espacioFisicoInicial,
    );

    this.formularioRestriccionesLugarTiempo = this.formBuilder.group({
      espacioFisico: [
        this.espaciosFisicosDisponibles.length
          ? espacioFisicoInicialObject[0]
          : "",
        [Validators.required],
      ],
      dia: [
        this.restriccionesPorActividad.length
          ? this.restriccionesPorActividad[0].dia
          : "",
        [Validators.required],
      ],
      hora: [
        this.restriccionesPorActividad.length
          ? this.restriccionesPorActividad[0].hora
          : "",
        [Validators.required],
      ],
    });
  ...
  crearRestriccion() {
    const restriccion = {
      ...
      idEspacioFisico:
        this.formularioRestriccionesLugarTiempo.value.espacioFisico.id,
      ...
      error: (error) => {
        Swal.fire({
          ...
          text: !error.error.data
            ? error.error.message
            : `Restricción ya fue asignada a la actividad:
  ...
  cargarRestricciones(idActividad: number) {
    this.actividadService.obtenerRestriccionesPorId(idActividad).subscribe({
      next: (data) => {
